...
Linux run `33951142319` subsequently passed strict regeneration, proving the normalized files match across platforms. Its browser run passed 141/142; the remaining test incorrectly expected full effects on a low-core runner, although production intentionally selects energy-saving below six cores. Reproducing with two emulated cores produced the same failure locally. The test now covers both two-core energy-saving and eight-core full-effects restoration, retaining the assertions that reduced motion remains held throughout active processing. No production effect behavior or timeout was changed.

The next Linux browser run passed 142/143; the pending-packaging RPC replacement case exceeded 15 seconds after previously completing in 12.1 seconds. Its setup cylinder was reduced from 256 to 64 segments, while retaining two real conversions, actual worker termination, the superseded error, and the 15-second timeout. An additional assertion proves packaging is still pending when replacement starts. This protocol test does not establish that PDF CPU work has started; the unchanged feature-rich packaging E2E provides that separate coverage. The focused revised test passed in 9.14 seconds locally and independent review found no blocking issues.
