...
`);
  ...
  it('runs the locked Rust, WASM, differential browser, bundle and release verifier gates in CI', () => {
    ...
    expect(ci).toContain('generate-wasm-release-evidence.ts docs/validation/wasm-geometry-release-measurements.jsonl');
    expect(ci).toContain('diff -u docs/validation/wasm-geometry-release-evidence.json');
    expect(ci).toContain('Privacy scan release evidence');
  ...
  it('does not deploy without the same locked Rust and software release contract gates', () => {
    ...
    expect(pages).toContain('generate-wasm-release-evidence.ts docs/validation/wasm-geometry-release-measurements.jsonl');
    expect(pages).toContain('diff -u docs/validation/wasm-geometry-release-evidence.json');
    expect(pages).toContain('Privacy scan release evidence');
