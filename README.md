...
三爪樣板 v3 優先採用使用者已試裝成功的 DXF，保留三組相隔 120° 的圓弧、端部圓角和短直線；內半徑為 10.5 mm，外半徑約 14 mm。來源實物為三層各 6 mm 壓克力。模板具有版本及 fingerprint，更新後舊專案須重新運算。

配合可由 -0.20 mm 至 +0.20 mm、每步 0.01 mm：正數較鬆，負數較緊。既有 0.20 mm 裝配餘量及 kerf 補償仍會套用，首次製作請先使用三爪尺寸測試片。來源實物的成功不代表所有新模型或其他板厚均已通過試裝。規劃會檢查 kerf、裝配餘量、外框包含、孔間距及最小剩餘結構；不安全時停止或省略不可靠的可選幾何。完整來源及解析記錄見[DXF 模板 v3](docs/validation/dxf-launcher-v3.md)。
